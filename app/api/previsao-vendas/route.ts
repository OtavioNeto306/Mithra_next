  ...
  try {
    const query = `
      SELECT 
        ...
        SUM(CASE WHEN P.TOTA_PED IS NOT NULL AND TRIM(P.TOTA_PED) != '' THEN TO_NUMBER(REPLACE(P.TOTA_PED, ',', '.')) ELSE 0 END) AS TOTAL_VENDAS,
        SUM(CASE WHEN I.QTDE_IPE IS NOT NULL AND TRIM(I.QTDE_IPE) != '' THEN TO_NUMBER(REPLACE(I.QTDE_IPE, ',', '.')) ELSE 0 END) AS TOTAL_ITENS,
        AVG(CASE WHEN P.TOTA_PED IS NOT NULL AND TRIM(P.TOTA_PED) != '' THEN TO_NUMBER(REPLACE(P.TOTA_PED, ',', '.')) ELSE 0 END) AS MEDIA_POR_PEDIDO
      FROM PEDIDO P
  ...
  try {
    const query = `
      SELECT 
        ...
        CASE WHEN P.TOTA_PED IS NOT NULL AND TRIM(P.TOTA_PED) != '' THEN TO_NUMBER(REPLACE(P.TOTA_PED, ',', '.')) ELSE 0 END AS TOTAL,
        CASE WHEN P.VFRT_PED IS NOT NULL AND TRIM(P.VFRT_PED) != '' THEN TO_NUMBER(REPLACE(P.VFRT_PED, ',', '.')) ELSE 0 END AS FRETE,
  ...
  try {
    const query = `
      SELECT 
        ...
        CASE WHEN I.QTDE_IPE IS NOT NULL AND TRIM(I.QTDE_IPE) != '' THEN TO_NUMBER(REPLACE(I.QTDE_IPE, ',', '.')) ELSE 0 END AS QUANTIDADE,
        CASE WHEN I.VLOR_IPE IS NOT NULL AND TRIM(I.VLOR_IPE) != '' THEN TO_NUMBER(REPLACE(I.VLOR_IPE, ',', '.')) ELSE 0 END AS VALOR_UNITARIO,
        CASE WHEN I.DSAC_IPE IS NOT NULL AND TRIM(I.DSAC_IPE) != '' THEN TO_NUMBER(REPLACE(I.DSAC_IPE, ',', '.')) * -1 ELSE 0 END AS DESCONTO,
        CASE WHEN I.VLIQ_IPE IS NOT NULL AND TRIM(I.VLIQ_IPE) != '' THEN TO_NUMBER(REPLACE(I.VLIQ_IPE, ',', '.')) ELSE 0 END AS TOTAL_ITEM,
        I.CCFO_CFO AS CFOP,
...
async function getProdutosMaisVendidos(vendedor: string) {
  try {
    const query = `
      SELECT 
        I.CODI_PSV AS PRODUTO,
        SUM(CASE WHEN I.QTDE_IPE IS NOT NULL AND TRIM(I.QTDE_IPE) != '' THEN TO_NUMBER(REPLACE(I.QTDE_IPE, ',', '.')) ELSE 0 END) AS TOTAL_QUANTIDADE
      FROM IPEDIDO I
      JOIN PEDIDO P ON P.PEDI_PED = I.PEDI_PED AND P.SERI_PED = I.SERI_PED
      WHERE P.DEMI_PED >= SYSDATE - 31
        AND P.SERI_PED = '9'
  ...
  try {
    const query = `
      SELECT 
        ...
        SUM(CASE WHEN P.TOTA_PED IS NOT NULL AND TRIM(P.TOTA_PED) != '' THEN TO_NUMBER(REPLACE(P.TOTA_PED, ',', '.')) ELSE 0 END) AS TOTAL_DIA
      FROM PEDIDO P
      WHERE P.DEMI_PED >= SYSDATE - 31
        AND P.SERI_PED = '9'
        AND P.COD1_PES = ?
